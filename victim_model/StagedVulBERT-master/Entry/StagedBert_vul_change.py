...
def convert_examples_to_features(data):
    func, label_c, label_f, tokenizer, args, index = data
    # source
    rows = str(func).split('\n')
    ...
    return InputFeatures(source_tokens, tokens_features, row_num, label_c, label_f, index)
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='', file_type="train"):
        ...
        if os.path.exists(data_path):
            ...
            labels_f = [None] * len(df)
            indices = df["index"].tolist()
            tokenizers = [tokenizer] * len(funcs)
            arg = [args] * len(funcs)
            source = list(zip(funcs, labels_c, labels_f, tokenizers, arg, indices))
    ...
    def __getitem__(self, i):
        ...
        d = torch.tensor(self.examples[i].index)
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            (labels, _, _) = [x.to(args.device) for x in batch[1:]]
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False, best_threshold=0.5):
    ...
    for batch in bar:
        (labels, _, index) = [x.to(args.device) for x in batch[1:]]
        tokens_features = [[y.to(args.device) for y in x] for x in batch[0]]
        with torch.no_grad():
    ...
    if not eval_when_training:
        ...
        record_df['logits'] = logits[:, 1]
        record_df['y_true'] = y_trues
        record_df['y_pred'] = y_preds
        record_df.to_csv(f'{args.model_name}_test_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', index=False)
...
def main():
    ...
    # parameters
    parser.add_argument("--train_data_file", default="../../DataPoisoningVulnerabilityDetection/Data/importance_500_atg_sim_train_position_ATGNet_merge_max_random_important_token_filter_random_0.15_1e-6_0.9_0.1.csv", type=str, required=False,
    ...
    parser.add_argument("--eval_data_file", default="../../LineVul/LineVul-main/data/big-vul_dataset/val.csv", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--test_data_file", default="../../LineVul/LineVul-main/data/big-vul_dataset/test.csv", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")

    parser.add_argument("--model_name", default="model_2048.bin", type=str,
                        help="Saved model name.")
    parser.add_argument("--model_name_or_path", default="../../DataPoisoningVulnerabilityDetection/codebert-base", type=str,
                        help="The model checkpoint for weights initialization.")
    parser.add_argument("--config_name", default="../../DataPoisoningVulnerabilityDetection/codebert-base", type=str,
                        help="Optional pretrained config name or path if not the same as model_name_or_path")
    parser.add_argument("--use_non_pretrained_model", action='store_true', default=False,
                        help="Whether to use non-pretrained model.")
    parser.add_argument("--tokenizer_name", default="../../DataPoisoningVulnerabilityDetection/codebert-base", type=str,
