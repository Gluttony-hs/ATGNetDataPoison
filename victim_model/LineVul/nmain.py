...
def test(args, model, tokenizer, best_threshold=0.5):
    ...
    funs = """getftp (struct url *u, wgint passed_expected_bytes, wgint *qtyread,
...
"""
    label = 1
    x = convert_examples_to_features(funs, label, tokenizer, args)
    inputs_ids = torch.tensor(x.input_ids).unsqueeze(0).to(args.device)
    labels = torch.tensor(x.label).unsqueeze(0).to(args.device)
    with torch.no_grad():
        lm_loss, logit = model(input_ids=inputs_ids, labels=labels)
        logits.append(logit.cpu().numpy())
    # calculate scores
...
def main():
    ...
    args.test_data_file="../data/big-vul_dataset/test.csv"
    args.block_size=512
